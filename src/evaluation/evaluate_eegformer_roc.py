...
def main():
    ...
    print("=" * 60)
    print("EEGFORMER ROC-AUC EVALUATION")
    print("=" * 60)
    print("Device:", device)
    ...
    for class_index in range(NUM_CLASSES):
        ...
        plt.plot(
            fpr,
            tpr,
            label=f"{LABEL_NAMES[class_index]} AUC = {roc_auc:.3f}",
        )

        print(
            f"{LABEL_NAMES[class_index]} ROC-AUC: {roc_auc:.4f}"
        )
    ...
    plt.title("EEGFormer ROC Curves")
    plt.legend()
    plt.tight_layout()

    output_path = FIGURE_DIR / "eegformer_roc_curve.png"
